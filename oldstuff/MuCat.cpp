...
\
	...
	// Get i/o access
	HANDLE h_giveio = CreateFile("\\\\.\\giveio",GENERIC_READ,0,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
	if (h_giveio == INVALID_HANDLE_VALUE)
		Texit("Couldn't access GIVEIO device.");
	...
	pDoc->SetGain();
	pDoc->SetMaxHeight();
	pDoc->GetSlide()->SetAcceleration(200000);
	pDoc->GetTurntable()->FindHome();
	pDoc->GetSlide()->FindHome();
	pDoc->GetElevator()->FindHome();
//	pDoc->GetSlide()->SetAcceleration(100000);
	pDoc->SetTheta(pDoc->GetHeader()->theta/1000.0);
	pDoc->GetTurntable()->Move(-pDoc->GetHeader()->theta);
	pDoc->SetElevation(pDoc->GetHeader()->z);
	pDoc->GetElevator()->Move(pDoc->GetElevation()*(-30));
